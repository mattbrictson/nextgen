...
Rake::Task["release"].enhance do
  ...
  system "open https://github.com/mattbrictson/nextgen/releases"
end
...
task :verify_gemspec_files do
  ...
  gemspec_files = Gem::Specification.load("nextgen.gemspec").files.sort
  ignored_by_git = gemspec_files - git_files
  next if ignored_by_git.empty?

  raise <<~ERROR

    The `spec.files` specified in nextgen.gemspec include the following files
